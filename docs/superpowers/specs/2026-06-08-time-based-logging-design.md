...
- 記録行の表示：右側に `{minutes}分`、左側に部位名（複数は「胸・腕」のように `・` 連結）。部位未選択なら部位名は表示せず、時間だけを表示する（補完テキストは入れない）。
- ログカードの件数ラベル「{n} 種目」→「{n} きろく」。
- 空状態メッセージ「やったことを じぶんで書こう！」→「きょうの トレを きろくしよう！」（種目入力前提の文言を修正）。

### カレンダー（calendar.jsx）

- 行表示 `{e.amount}{e.unit}` → `{e.minutes}分`、種目名 `{e.name}` → 部位（複数は `・` 連結、未選択は部位名なしで時間だけ表示）。
